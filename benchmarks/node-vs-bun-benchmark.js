...
/**
 * Node.js vs Bun + Node-API Performance Benchmark
 *
 * This benchmark compares the performance of our Rust core engine
 * when called from Node.js vs Bun using Node-API.
 */
...
function benchmarkWorkflowRegistration(iterations = 100) {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    const result = core.registerWorkflow(workflowJson, './benchmark-node.db');
...
function benchmarkStatusRetrieval(iterations = 100) {
  ...
  const registerResult = core.registerWorkflow(
    workflowJson,
    './benchmark-node.db'
  );
  ...
  for (let i = 0; i < iterations; i++) {
    const result = core.getRunStatus(runId, './benchmark-node.db');
...
function benchmarkStepExecution(iterations = 50) {
  ...
  for (let i = 0; i < iterations; i++) {
    const result = core.executeStep(runId, 'step1', './benchmark-node.db');
...
function benchmarkMemoryUsage() {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    core.registerWorkflow(workflowJson, './benchmark-node.db');
  }
...
async function runBenchmarks() {
  console.log('🚀 Starting Node.js + Node-API Performance Benchmark');
